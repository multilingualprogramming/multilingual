...
class JavaScriptCodeGenerator:
    ...
    def _emit_core_runtime(self) -> None:
        """Emit scalar and type helpers for the generated JS runtime."""
    ...
    def _emit_collection_runtime(self) -> None:
        """Emit collection helpers for the generated JS runtime."""
        self._emit(
        ...
        self._emit(
            "if (Array.isArray(value) || typeof value === 'string' || value instanceof Set) "
            "return value;"
        )
    ...
    def _emit_random_module(self) -> None:
        """Emit the Python-like ``random`` module facade."""
    ...
    def visit_ForLoop(self, node) -> None:
        ...
        self._emit(f"for (const {target} of __ml.iter({iterable})) {{")
        self._emit_body(node.body)
        self._emit("}")
...
class _JSExpressionGenerator:
    ...
    def visit_CallExpr(self, node):
        ...
        special_call = self._special_identifier_call(node)
        if special_call is not None:
            return special_call
        args = [self._expr(arg) for arg in node.args]
        if isinstance(node.func, AttributeAccess):
            return self._attribute_call(node.func, args)
        return f"{func}({', '.join(args)})"

    def _special_identifier_call(self, node):
        """Return JS for identifier calls needing Python-compatible lowering."""
        if isinstance(node.func, Identifier) and node.func.name == "isinstance":
            return self._isinstance_call(node)
        if isinstance(node.func, Identifier) and node.func.name in {"max", "min"}:
            return self._keyed_extreme_call(node)
        return None

    def _isinstance_call(self, node):
        if len(node.args) < 2:
            return None
    ...
    def _keyed_extreme_call(self, node):
        key_expr = None
        for name, value in node.keywords:
            if name == "key":
                key_expr = self._expr(value)
        if not key_expr or not node.args:
            return None
    ...
    def _attribute_call(self, func, args):
        obj = self._expr(func.obj)
        attr = func.attr
        direct_helpers = {
            "get": f"__ml.get({obj}, {', '.join(args)})",
            "items": f"__ml.items({obj})",
            "keys": f"Object.keys({obj} ?? {{}})",
            "values": f"Object.values({obj} ?? {{}})",
            "append": f"__ml.append({obj}, {args[0] if args else 'undefined'})",
            "index": f"{obj}.indexOf({args[0] if args else 'undefined'})",
            "replace": f"{obj}.replaceAll({', '.join(args)})",
            "join": f"({args[0] if args else '[]'}).join({obj})",
            "split": f"{obj}.split({args[0] if args else 'undefined'})",
            "strip": f"{obj}.trim()",
            "is_integer": f"Number.isInteger({obj})",
        }
        if attr in direct_helpers:
            return direct_helpers[attr]
        if attr == "choice":
            return self._random_attr_call(func, obj, args, attr)
        if attr == "random":
            return self._random_attr_call(func, obj, args, attr)
        return f"{obj}.{attr}({', '.join(args)})"

    def _random_attr_call(self, func, obj, args, attr):
        is_random_module = isinstance(func.obj, Identifier) and func.obj.name == "random"
        if attr == "choice":
            if is_random_module:
    ...
    def visit_LambdaExpr(self, node):
        params = ", ".join(
            param if isinstance(param, str) else self._expr(param)
