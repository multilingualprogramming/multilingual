...
def _method_aliases_for(concept: str) -> frozenset[str]:
    return frozenset(_UI_LOWERING_CONFIG.get("method_aliases", {}).get(concept, []))


def _ui_aliases_for(section: str, concept: str) -> frozenset[str]:
    return frozenset(_UI_LOWERING_CONFIG.get(section, {}).get(concept, []))
...
_UI_TYPE_ALIASES = _UI_RUNTIME_CONFIG.get("type_aliases", {})
...
class UILoweringPass:
    ...
    def _emit_preamble(self) -> str:
        ...
        type_alias_lines = []
        for type_name, aliases in _UI_TYPE_ALIASES.items():
