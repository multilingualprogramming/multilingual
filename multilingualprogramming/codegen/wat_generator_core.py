...
class WATGeneratorCoreMixin:
  ...
  ;; ── SIMD (v128 / f64x2) Mandelbrot pair kernel ───────────────────────────
  ;; Itère deux pixels Mandelbrot en parallèle via WebAssembly SIMD (v128).
  ;; Sur les CPU x86-64 (SSE2) et ARM64 (NEON), chaque instruction f64x2.*
  ;; traite deux lanes en un seul cycle — gain ~2× sur la boucle interne.
  ;;
  ;; Sémantique : pour chaque lane indépendamment, équivaut à mandelbrot(c, max_iter)
  ;; tel que défini dans fractales_escape.multi (rayon d'évasion² = 4.0). Les lanes
  ;; qui s'échappent en premier sont « gelées » via bitselect — le compteur d'itérations
  ;; n'avance que sur les lanes encore actives. Retourne deux entiers (en f64) :
  ;; itérations_lane0, itérations_lane1.
  ;;
  ;; Retour : pointeur f64 vers une liste de 2 cellules f64 = [iter0, iter1].
  ;; Layout de la liste (convention multilingual) : header i32 à ptr-8 (= 2.0
  ;; comme f64 à offset 0), items à ptr-0 et ptr+8.
  (func $mandelbrot_pair_simd
        (param $cx0 f64) (param $cy0 f64)
        (param $cx1 f64) (param $cy1 f64)
        (param $max_iter f64)
        (result f64)
    (local $vzx v128) (local $vzy v128)
    (local $vcx v128) (local $vcy v128)
    (local $vr2 v128) (local $vactive v128)
    (local $vnzx v128) (local $vnzy v128)
    (local $viter v128)
    (local $k i32) (local $kmax i32) (local $any i32)
    (local $two_f64x2 v128) (local $four_f64x2 v128) (local $one_i64x2 v128)
    (local $buf i32) (local $iter0 f64) (local $iter1 f64)
    ;; Construire les vecteurs initiaux à partir des params.
    ...
    f64x2.replace_lane 1
    local.set $vcy
    ;; z₀ = 0+0i sur les deux lanes.
    v128.const f64x2 0 0
    local.set $vzx
    v128.const f64x2 0 0
    local.set $vzy
    ;; Compteur d'itérations par lane (i64x2, démarre à 0).
    v128.const i64x2 0 0
    local.set $viter
    ;; Constantes vectorielles.
    v128.const f64x2 2 2
    local.set $two_f64x2
    v128.const f64x2 4 4
    local.set $four_f64x2
    v128.const i64x2 1 1
    local.set $one_i64x2
    ;; Borne d'itération en i32 (truncation signée — assez large pour max_iter < 2^31).
    local.get $max_iter
    i32.trunc_f64_s
    local.set $kmax
    i32.const 0
    ...
    block $done
      loop $boucle
        ...
        f64x2.add
        local.set $vr2
        ;; active_mask = (r² ≤ 4) — résultat lane = all-1s si non échappé.
        ;; Utilise `le` (et non `lt`) pour matcher la sémantique scalaire
        ;; `r² > 4 stop` : à r²=4 exactement, l'orbite continue.
        local.get $vr2
        local.get $four_f64x2
        f64x2.le
        local.set $vactive
        ;; Sortir si toutes les lanes ont échappé.
        local.get $vactive
        v128.any_true
        local.set $any
        local.get $any
        i32.eqz
        br_if $done
        ;; iter += 1 sur les lanes actives :
        ;;   ones_per_active_lane = active_mask AND i64x2(1, 1)
        ;;   iter = iter + ones_per_active_lane
        local.get $viter
        local.get $vactive
        local.get $one_i64x2
        v128.and
        ...
        f64x2.add
        local.set $vnzy
        ;; Geler les lanes échappées : zx = active ? nzx : zx ; idem zy.
        local.get $vnzx
        local.get $vzx
        local.get $vactive
        v128.bitselect
    ...
    ;; Extraire les itérations finales (f64 pour la convention multilingual).
    local.get $viter
    ...
    f64.convert_i64_s
    local.set $iter1
    ;; Allouer une liste de 2 f64 (suit la convention multilingual : header
    ;; i32 à ptr-8 contenant la longueur sous forme f64). On utilise
    ;; ml_alloc(16) puis on écrit [iter0, iter1] aux offsets 0, 8.
    i32.const 24
    call $ml_alloc
    local.set $buf
    ;; Header : 2.0 (f64) à $buf — convention header f64 longueur.
    local.get $buf
    f64.const 2
    f64.store
    ;; iter0 à $buf+8
    ...
    local.get $iter1
    f64.store
    ;; Retourne le pointeur vers la première cellule (header inclus comme
    ;; en `[0]*n` standard). La convention dans le projet : le pointeur retourné
    ;; pointe au header, et le caller lit sortie[0] à ptr+8 (cf. lectures
    ;; orbite/atelier). Cohérent avec `[0.0]*n`.
    local.get $buf
    f64.convert_i32_s
  )
